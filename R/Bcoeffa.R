      ...
      z=z[igood]  
       ...
       xd[imissx]=0                      
          ...
          for (k in 1:numtau)
           ...
           if (lag < 0)
              ...
              ahat[evenind,k]=2*Re(Bkhat[(2:(kupper+1)),k])  
              ahat[oddind,k]=-2*Im(Bkhat[(2:(kupper+1)),k])    
  
          if (T%/%2)  {ahat[T,k]= Re(Bkhat[(T/2)+1,k]) }         
       ...
       if ( printflg)
         ...
         {  cat(paste('Bcoeffs in ab form for',datastr, 'lag=',tau[k],'\n'))
            cat('k  ahat_k n1    n2   Fratio   pv','\n')
            for (kk in 1:dim(pvec)[1])
         { cat(paste(kk-1,ahat[kk,k],n1[kk,k],n2[kk,k],fratio[kk,k], pvec[kk,k],'\n')) }
     }
   }
...
L<-modifyList(list(printflg=1,meth=0), list(x = x, T=T, tau=tau, missval=missval, datastr=datastr,...))
