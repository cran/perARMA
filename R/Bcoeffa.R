Bcoeffa<-function (x, T_t, tau, missval, datastr, ...)
{
    Bcoeffa_full <- function(x, T_t, tau, missval, datastr, printflg,
        meth) {
        nout = floor((T_t + 2)/2)
        nx = length(x)
        ...
        ahat <- matrix(0, T_t, numtau)
        n1 <- matrix(1, T_t, numtau)
        n2 <- matrix(1, T_t, numtau)
        fratio <- matrix(1, T_t, numtau)
        pvec <- matrix(1, T_t, numtau)
        ...
        if (printflg) {
            for (k in 1:numtau) {
               message(paste("\n"))
               message(paste("Bcoeffs in ab form for", datastr,"lag=", tau[k], "\n"))
